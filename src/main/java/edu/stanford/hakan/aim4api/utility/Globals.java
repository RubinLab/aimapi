...
//        	Logger.write("Couldn't read from proxy-config. Using default namespace gme://caCORE.caCORE/4.4/edu.northwestern.radiology.AIM");
        	}
        return namespaceV4;
    }
...
//	    	Logger.write("Couldn't read from proxy-config. Using default xsdfile AIM_v4_rv44_XML.xsd");
	    	}
    	return xsdFileV4;
    }
...
//        return getFile(listResources, listConfigVariables, 1);
        String aimQLXml= "AimXPath.xml";
	    try{ 
	    	aimQLXml=EPADConfig.getInstance().getStringPropertyValue("baseSchemaDir")+EPADConfig.getInstance().getStringPropertyValue("aimQLXml");
	    }catch(Exception e){
//	    	Logger.write("Couldn't read from proxy-config. Using default aimQLXml AimXPath.xml");
	    	}
    	return aimQLXml;
...
//        return getFile(listResources, listConfigVariables, 1);
        ...
        String annotationsList= "AnnotationsList.txt";
	    try{ 
	    	annotationsList=EPADConfig.getInstance().getStringPropertyValue("baseSchemaDir")+EPADConfig.getInstance().getStringPropertyValue("annotationsList");
	    }catch(Exception e){
//	    	Logger.write("Couldn't read from proxy-config. Using default annotationsList AnnotationsList.txt");
	    	}
...
//        return getFile(listResources, listConfigVariables, 1);
    	return EPADConfig.getInstance().getStringPropertyValue("baseSchemaDir")+getXsdFileName();
    }
...
//        return getFile(listResources, listConfigVariables, 1);
    	 ...
    	 String aimApiLog= "AIMAPI.log";
 	    try{ 
 	    	aimApiLog=EPADConfig.getInstance().getStringPropertyValue("baseSchemaDir")+EPADConfig.getInstance().getStringPropertyValue("aimApiLog");
 	    }catch(Exception e){
// 	    	Logger.write("Couldn't read from proxy-config. Using default aimApiLog AIMAPI.log");
 	    	}
     	return aimApiLog;
    }
}
