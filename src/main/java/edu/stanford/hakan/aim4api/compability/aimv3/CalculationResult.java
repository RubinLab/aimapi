/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */
package edu.stanford.hakan.aim4api.compability.aimv3;

import edu.stanford.hakan.aim4api.base.ST;

/**
 *
 * @author Hakan
 */
//public class CalculationResult extends edu.stanford.hakan.aim3api.base.CalculationResult {
public class CalculationResult {

//    public edu.stanford.hakan.aim4api.base.CalculationResult toAimV4() {
//        
//        edu.stanford.hakan.aim4api.base.CalculationResult res = new edu.stanford.hakan.aim4api.base.CalculationResult();
//        
//        //this.getCalculationDataCollection()
//        //this.getDimensionCollection()
//        //this.getNumberOfDimensions()        
//        //this.getCagridId() OK
//        //this.getType() OK
//        //this.getUnitOfMeasure() OK
//        
//        //res.setDataType(null);
//        //res.setDimensionCollection(null);
//        //res.setType(Enumerations.CalculationResultIdentifier.None);
//        //res.setUnitOfMeasure(null);
//        
//        
//        
//        res.setUnitOfMeasure(new ST(this.getUnitOfMeasure()));
//        res.setType(EnumerationConverter.calculationResultIdentifier(this.getType()));
//        
//        
//        
//        return res;
//    }
}
